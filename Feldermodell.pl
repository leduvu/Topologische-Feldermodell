...
% denn sobald die VP mit dem Verb beginnt, fŠngt auch die linke Satzklammer an.
% Alles vor der VP ist die NP und alles vor der Linkensatzklammer ist das Vorfeld.
%---------------------------------

satz(s / [NPBaum,VBaum]) -->
	vorfeld(NPBaum,agr(Person,Numerus,nom)),	% NP und VP mŸssen kongruent sein in Person und Numerus.
	lk1(VBaum,fin(Person,Numerus,_,_)).


%---------------------------------
% Da nicht die komplette VP zur linken Satzklammer gehšrt,
...
% VP --> V NP
% Ein Transitives Verb fordert eine NP und diese NP gehšrt zum Mittelfeld.
%---------------------------------

lk_mittelfeld(lk /[VBaum,NPBaum],Merkmale) -->
	v(VBaum,Merkmale,[regiert(Kasus)]),	% Der Kasus von Verb und der NP, die im Mittelfeld
	mittelfeld(NPBaum,agr(_,_,Kasus)).	% steht, mŸssen gleich sein.
...
vorfeld(vorfeld / [NBaum],agr(3,pl,Kasus)) -->
	n(NBaum,kng(Kasus,pl,_)).			% Nomen ohne Artikel mŸssen im Plural stehen.
...
%---------------------------------
% MITTELFELD
% Im Mittelfeld kšnnen auch viele verschiedene Wortarten stehen.
%---------------------------------
%---------------------------------
% Mittelfeld = N
% N
...
%---------------------------------
% Definition der PrŠpositionalphrase im Vorfeld und im Mittelfeld.
% Die PP braucht zur Definition auch die Definition der NP.
%---------------------------------

% PP --> P NP
pp([PBaum,NPBaum]) -->
	p(PBaum,[regiert(Kasus)]),		% Der Kasus der PrŠposition muss mit dem der NP Ÿbereinstimmen.
	np(NPBaum,agr(_,_,Kasus)).

% NP --> Art N
np([ArtBaum,NBaum],agr(3,Numerus,Kasus)) -->
	art(ArtBaum,KNG),				% Artikel und Nomen mŸssen die selben KNG Merkmale haben.
	n(NBaum,KNG),	  
	{ KNG = kng(Kasus,Numerus,_) }.
